...
--
-- This migration closes the remaining public-truth boundary gaps without changing
-- the Lineage V3 state machine or validation economics:
-- * one permanent-account eligibility definition owns all public-truth voting;
-- * INITIAL, CASCADE, and human V1 snapshots exclude anonymous profiles;
-- * CASCADE has its own protocol identifier instead of borrowing V1 identity;
-- * ordinary clients can no longer write public status transitions directly;
-- * lineage candidates must inherit the current logic-rule identity server-side;
-- * canonical knowledge titles use the same NFKC compatibility normalization as
--   the client before trimming, whitespace folding, and lower-casing.

-- ---------------------------------------------------------------------------
-- One server definition of an eligible public-truth participant.
-- ---------------------------------------------------------------------------
...
comment on function private.is_eligible_public_voter(uuid) is
  'Single authoritative eligibility predicate for public knowledge submission and truth voting.';
comment on function private.eligible_public_voter_count(timestamptz) is
  'Counts only active permanent accounts eligible to participate in public truth voting.';

-- Public event submission now consumes the same eligibility predicate instead of
-- maintaining an independent copy of the account-definition logic.
...
-- Revalidation initiation also consumes the same predicate. The trigger remains
-- the final authority even when an RPC or a future client path changes.
...
-- Every newly created pending-vote round gets an authoritative eligible-user
-- snapshot at its opened_at. This overrides stale counts computed by legacy RPCs.
-- CASCADE receives a protocol identity of its own at the same boundary.
...
-- Human ORIGINAL_DESIGN_V1 rounds use the same eligible population snapshot.
...
-- Defense in depth at the actual ballot tables: neither the INITIAL/CASCADE RPC
-- family nor the human V1 RPC family can persist a ballot for an anonymous or
-- otherwise ineligible account, even if a non-official client calls them directly.
...
-- CASCADE is not ORIGINAL_DESIGN_V1. Preserve historical audit rows but relabel
-- every existing cascade round and constrain all future protocol identities.
...
-- ---------------------------------------------------------------------------
-- Public event boundary hardening.
-- ---------------------------------------------------------------------------
