...
-- knowledge_dependency_edges stays the single authoritative logical DAG.
...
begin
  ...
  foreach node_id in array premise_ids loop
    if node_id=any(conclusion_ids) then raise exception 'a node cannot be both premise and conclusion: %',node_id using errcode='22023'; end if;
    select d.node_type into node_type from public.public_knowledge_node_declarations d
    ...
    latest_verdict:=null;
    select r.verdict into latest_verdict from public.knowledge_pending_vote_rounds r
    where r.node_id=node_id order by r.round_no desc,r.opened_at desc limit 1;
    if latest_verdict in ('PENDING','INCORRECT') then raise exception 'reasoning-link premise must already be verified: %',node_id using errcode='22023'; end if;
    lineage_role:=null;
    select lm.role into lineage_role from private.knowledge_lineage_members lm where lm.node_id=node_id;
    if lineage_role is not null and lineage_role<>'current' then raise exception 'reasoning-link premise must be the current lineage node: %',node_id using errcode='22023'; end if;
  end loop;
...
create or replace function private.project_reasoning_link_event()
returns trigger language plpgsql security definer
set search_path=private,public,pg_temp
as $$
begin
...
revoke all on function private.project_reasoning_link_event() from public,anon,authenticated;

drop trigger if exists project_reasoning_link_event on public.public_knowledge_events;
create trigger project_reasoning_link_event after insert on public.public_knowledge_events
for each row execute function private.project_reasoning_link_event();
