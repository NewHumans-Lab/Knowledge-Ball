...
“既有节点”包括已证伪、已合并、已分解、被替代和默认隐藏的历史节点。隐藏不是释放名称。编辑已有节点时，浏览器和服务端也不得把标题或描述改成其他节点已经占用的文本。

## 4. 命令与事件边界

四种用户编辑只能通过 `executeKnowledgeEdit` 写入。命令先用完整投影验证，再追加一个事件：

| 行为 | 命令 `kind` | 原子事件 |
| --- | --- | --- |
| 增加 | `add` | `KnowledgeAdded` |
| 否定 | `negate` | `KnowledgeNegated` |
| 分解 | `decompose` | `KnowledgeDecomposed` |
...
```text
原前提 -> 推理步骤一 -> 中间结论 -> 推理步骤二 -> 原结论
```

中间结论数量必须等于推理步骤数量减一。端点不完整、逻辑符号缺失、标题或描述重复时，不得写入任何新节点。成功后原推理节点进入 `suspended`、写入 `supersededBy` 并默认隐藏；原结论保留原 ID，只把直接前提切换到最后一个新推理步骤。
...
领域投影保存全部节点。默认 3D 投影只取 `hidden !== true` 的节点，所以否定、合并和分解不会造成历史数据消失，也不会让界面堆满被替代节点。
