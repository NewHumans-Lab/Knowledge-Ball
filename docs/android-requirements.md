...
网页设置面板提供同源 APK 下载地址 `./downloads/knowledge-ball-android-v0.2.0.apk`。安装包随网页静态产物一起部署，不依赖 Google Play、GitHub Releases 或其他需要跳转的下载服务；用户打开网页后即可直接下载。`CAPACITOR_BUILD=true` 时会禁用 Vite 的 `public` 目录复制，避免把 APK 再嵌套进 Android 应用自身。
