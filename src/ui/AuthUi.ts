...
function openAccount(shouldLoad = true): void {
  ...
  body.innerHTML = `
    <div class="kb-profile-head"><div class="kb-profile-avatar" id="kbProfileAvatar">${initial(cached)}</div><div><strong id="kbProfileName">${name(cached)}</strong><small>@${cached?.username ?? '设置用户名'}</small></div></div>
    <div class="kb-profile-bio">${cached?.bio ?? '匿名参与者也可以编辑知识、投票并设置公开资料。'}</div>
    ...
    <div class="form-hint kb-auth-status" id="kbAccountStatus">${account ? '正在自动同步账户数据…' : '远程服务未配置，本地知识功能仍可使用。'}</div>`;
...
function updateAvatar(): void { const avatar=document.querySelector<HTMLElement>('.avatar-btn');if(!avatar)return;avatar.textContent=initial(cached);avatar.title='个人空间 · 匿名参与';avatar.dataset.authState='anonymous'; }
...
function installStyles():void{const style=document.createElement('style');style.textContent=`.kb-profile-head{display:flex;align-items:center;gap:12px;margin-bottom:10px}.kb-profile-head small{display:block;color:var(--ink-faint);margin-top:3px}.kb-profile-avatar{width:48px;height:48px;border-radius:50%;display:grid;place-items:center;background:var(--bg-deep);border:1px solid var(--brass-dim);color:var(--brass);font-weight:700}.kb-profile-bio{font-size:12px;color:var(--ink-dim);line-height:1.6;margin-bottom:12px}.kb-account-main-action{width:100%;margin-top:10px}`;document.head.appendChild(style);}
