...
import { CORE_AMBIENT_LIGHT_INTENSITY, CORE_SUN_LIGHT_INTENSITY, CORE_SUN_RADIUS, DEFAULT_CAM_Z, LAYER_BANDS, MAX_GRAPH_ZOOM, MIN_GRAPH_ZOOM, NODE_LAYER_COLOR, NODE_SPECIAL_COLOR, PENDING_PULSE_FADE_MS, PENDING_PULSE_LOW_MS, PENDING_PULSE_MIN_OPACITY, PENDING_PULSE_MIN_SCALE, PENDING_PULSE_PERIOD_MS, PENDING_PULSE_RISE_MS, PENDING_PULSE_VISIBLE_MS, SUN_ORBIT_RADIUS, SUN_RADIUS_MM, SUN_TRIAD_IDS } from '../config/KnowledgeUiConfig';
...
assert(colorForNode(node('inner-definition','definition','verified'))===NODE_LAYER_COLOR.inner,'verified descriptive inner knowledge must use the inner cyan color');
assert(colorForNode(node('middle-theorem','theorem','verified'))===NODE_LAYER_COLOR.middle,'verified deterministic middle knowledge must use the middle electric-blue color');
