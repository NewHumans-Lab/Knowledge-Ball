...
assert(entry.includes('const globalMainChain = spatialKnowledge.filter(node => !isOrdinaryLineageSatellite(node))'),'ordinary history/opposition/candidates are solved after Current rather than entering initial main-chain occupancy');
assert(entry.indexOf('applyDeterministic5RLayout(globalMainChain)')<entry.indexOf('applyOrdinaryLineagePlacement(spatialKnowledge)'),'ordinary lineage must run only after initial main-chain geometry exists');
assert(entry.indexOf('applyOrdinaryLineagePlacement(spatialKnowledge)')<entry.indexOf('applyReasoningRadialPlacement(spatialKnowledge)'),'Reasoning projection must follow the final position of any ordinary served conclusion, including gray/red lineage balls');
