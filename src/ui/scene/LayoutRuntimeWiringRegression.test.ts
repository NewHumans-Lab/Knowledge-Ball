...
const reasoningBinding=readFileSync('src/domain/ReasoningConclusion.ts','utf8');
...
const ordinaryLineageFootprint=readFileSync('src/ui/scene/OrdinaryLineageFootprint.ts','utf8');
...
assert(entry.includes("from '../../domain/ReasoningConclusion'"),'reasoning ownership must be resolved in the semantic layer before geometry');
...
assert(entry.includes("from './OrdinaryLineagePlacement'"),'ordinary lineage role classification remains isolated from global scoring');
assert(entry.includes("from './OrdinaryLineageFootprint'"),'ordinary Knowledge lineage must enter the main solve through a pre-solved local footprint');
assert(entry.includes('bindReasoningConclusions(nodes)'),'one semantic Reasoning→conclusion binding must feed both layout and visibility');
assert(entry.includes('const globalMainChain = spatialKnowledge.filter(node => !isOrdinaryLineageSatellite(node))'),'ordinary history/opposition/candidates remain excluded from real-edge scoring nodes and enter as footprint occupancy instead');
assert(entry.indexOf('createOrdinaryLineageFootprintPlanner(spatialKnowledge)')<entry.indexOf('applyDeterministic5RLayout(globalMainChain, { footprintPlanner, footprintSignature })'),'ordinary lineage footprint planning must be installed before the unchanged global compact solve');
...
assert(implementation.includes('placed:[...path.placed,placement,...footprint]'),'local footprint cells must be reserved inside the same candidate path as their Current anchor');
...
assert(reasoningProjection.includes('conclusion.pos.clone().normalize()'),'Reasoning direction must be owned by the one served conclusion radial axis');
assert(!reasoningProjection.includes('meanRadius(conclusions)'),'multi-conclusion centroid projection must not return');
...
console.log('Lineage-first footprint + unchanged hierarchical radial ISG runtime architecture checks passed.');
