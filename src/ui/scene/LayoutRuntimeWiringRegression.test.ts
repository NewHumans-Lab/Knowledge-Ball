...
assert(entry.includes('const globalMainChain = spatialKnowledge.filter(node => !isOrdinaryLineageSatellite(node))'),'ordinary history/opposition/candidates are solved after Current rather than perturbing its global anchor');
...
assert(entry.indexOf('applyReasoningRadialPlacement(globalMainChain)')<entry.indexOf('applyOrdinaryLineagePlacement(spatialKnowledge)'),'ordinary lineage must be placed after the main-chain and existing Reasoning geometry are final');
