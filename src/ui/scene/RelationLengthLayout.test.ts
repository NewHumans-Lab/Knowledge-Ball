...
assert(lengthAfter <= lengthBefore + 1e-6, 'accepted optimization must never increase total displayed line length');
...
assert(Math.abs(result.before - lengthBefore) < 1e-6 && Math.abs(result.after - lengthAfter) < 1e-6, 'reported objective must match the actual Bézier line total');
...
console.log('Complete historical relation-length layout regression tests passed.');
