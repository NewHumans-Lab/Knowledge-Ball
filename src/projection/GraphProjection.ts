  ...
  apply(event: DomainEvent): void {
    ...
    switch (event.type) {
      case 'NodeCreated': {
        ...
        this.state.nodesById[p.nodeId] = { id: p.nodeId, title: p.title, type: p.nodeType, status: 'pending', mastery: 'none', reasoning: p.reasoning, premises: [...p.premises] };
        break;
      }
      case 'NodeEdited': {
        const p = event.payload;
      ...
      case 'NodeFalsified': { const n = this.state.nodesById[event.payload.nodeId]; if (n) n.status = 'falsified'; break; }
